...
| F8 | Regroupement de lignes + agrégats | Vérifié | Logique pure dans `row-grouping.ts` (groupes, unités de pagination, en-têtes, agrégats) ; `displayedRows()` reste `T[]`, les en-têtes passent par une source interne ; ligne de totaux ; vues ; 4 tests purs + 6 tests composant ; testé dans la démo |
